...
package com.jortilles.waqe.util;

import com.jortilles.waqe.WaqeConstants;
...
public class Utils {
  	...
  	public static IBasicFile getFile( String filePath, String basePath ) {
		...
		if( res.startsWith( WaqeConstants.SYSTEM_DIR + "/" ) ) {
			res = StringUtils.strip( res, WaqeConstants.SYSTEM_DIR + "/" );
			if( res.startsWith( WaqeConstants.PLUGIN_SYSTEM_DIR.toLowerCase() + "/" ) ) {
				filePath = filePath.replaceFirst( WaqeConstants.SYSTEM_DIR + "/" + WaqeConstants.PLUGIN_SYSTEM_DIR + "/", "" );
				return factory.getPluginSystemReader( basePath ).fetchFile( filePath );
			} else {
				String pluginId = res.substring( 0, filePath.indexOf( "/" ) );
				filePath = filePath.replaceFirst( WaqeConstants.SYSTEM_DIR + "/" + pluginId + "/", "" );
				return factory.getOtherPluginSystemReader( pluginId, basePath ).fetchFile( filePath );
			}
		} else if( res.startsWith( WaqeConstants.PLUGIN_REPOSITORY_DIR.toLowerCase() + "/" ) ) {
			filePath = filePath.replaceFirst( WaqeConstants.PLUGIN_REPOSITORY_DIR + "/", "" );
	...
	public static IRWAccess getSystemOrUserRWAccess( String filePath ) {
		...
		if( filePath.startsWith( "/" + WaqeConstants.SYSTEM_DIR + "/" ) && filePath.endsWith( WaqeConstants.FILE_EXTENSION ) ) {
